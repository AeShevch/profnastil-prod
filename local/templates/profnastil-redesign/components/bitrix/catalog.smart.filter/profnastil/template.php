...
<script>
    jQuery(document).on('click', ".btn-remove", function () {
        var id = jQuery(this).data('control');
        jQuery("#" + id).trigger('click');
        setTimeout(function () {
            jQuery('#set_filter').trigger('click');
        }, 800);

    });

    jQuery(document).on('click', ".remove_all", function () {
        ...
        <form name="<? echo $arResult["FILTER_NAME"] . "_form" ?>" action="<? echo $arResult["FORM_ACTION"] ?>"
            ...
            foreach ($arResult["ITEMS"] as $key => $arItem) {
                ...
                if (isset($arItem["PRICE"])):
                    ...
                    <div class="bx_filter_parameters_box active">
                        ...
                        <details class="main-filter__accordion accordion" open>
            ...
            foreach ($arResult["ITEMS"] as $key => $arItem) {
                ...
                <div class="bx_filter_parameters_box <? if ($arItem["DISPLAY_EXPANDED"] == "Y"):?>active<?endif ?>">
                    ...
                    ?><!--</div>-->
                    <details
                            class="bx_filter_block main-filter__accordion accordion" <?= $arItem['DISPLAY_EXPANDED'] == 'Y' ? 'open' : '' ?>>
                        <summary
